...
def print_number(number, char):
    ...
    patterns = digit_map(char)
    lines = []

    #moj przeglad typow jak sie zmieniaja
    print(type(patterns))
    print(type(patterns[1]))
    print(type(patterns[1][0]))

    lines.append(char * 40)
    lines.append(char + (" " * 38) + char)

    for row in range(6):
            line = "  "
            for digit in digits:
                 line += patterns[int(digit)][row] + " "
    ...
    return "\n".join(lines)
print(print_number(number, char))
...
def printNumber(number, char):
    ...
    # szerokość cyfr + odstępy między nimi
    content_width = digit_width * digit_count + (digit_count - 1)

    # padding poziomy (2 z każdej strony)
    horizontal_padding = 2
    inner_width = content_width + horizontal_padding * 2

    # ramka (1 znak z każdej strony)
    total_width = inner_width + 2

    # --- budowa ramek ---
    top_bottom = char * total_width
    empty_line = char + " " * (total_width - 2) + char
    ...
    for row in range(digit_height):
        ...
        for d in digits:
            pattern_row = DIGITS[int(d)][row]
            rendered_row = pattern_row.replace("X", char)
            rendered_digits.append(rendered_row)

        joined_digits = " ".join(rendered_digits)
        padded_line = " " * horizontal_padding + joined_digits + " " * horizontal_padding

        lines.append(char + padded_line + char)
